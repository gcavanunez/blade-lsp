...
import Blade = require("tree-sitter-blade");
...
export namespace BladeParser {
  ...
  /**
   * Initialize the tree-sitter parser with the Blade grammar
   */
  export function initialize(): Parser {
    ...
    parserInstance.setLanguage(Blade);
    return parserInstance;
  }
  ...
  export function getParser(): Parser {
    if (!parserInstance) {
      throw new Error("Parser not initialized. Call BladeParser.initialize() first.");
    }
    return parserInstance;
  }
  ...
  export function findNodeAtPosition(
    ...
    return findNodeAtPositionRecursive(rootNode, row, column);
  }
  ...
  function collectDirectives(node: SyntaxNode, directives: SyntaxNode[]): void {
    if (
      node.type === "directive" ||
      node.type === "directive_start" ||
      node.type === "directive_end"
    ) {
  ...
  function checkForErrors(node: SyntaxNode, diagnostics: DiagnosticInfo[]): void {
    // In tree-sitter 0.20.x, hasError and isMissing are functions
