...
export namespace Shared {
  ...
  export function findParentComponent(source: string, currentLine: number): string | null {
    ...
    for (let i = currentLine; i >= 0; i--) {
      ...
      // Count closing tags (excluding x-slot)
      const closingTags = line.match(/<\/x-(?!slot\b)[\w.-]+>/g);
      ...
      // Find opening tags (excluding x-slot)
      const openingMatch = line.match(/<(x-(?!slot\b)[\w.-]+|[\w]+:[\w.-]+)(?:\s|>)/);
      if (openingMatch) {
        if (depth === 0) {
